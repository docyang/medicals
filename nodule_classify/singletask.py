...
Wb = {
    ...
    #'W5': tf.get_variable('W5', [3, 3, 3, 32, 32], tf.float32, XAVIER_INIT),
...
def model(data, keep_prob):
  with tf.variable_scope('conv1') as scope:
    ...
    tf.add_to_collection('relu1', relu1)
  ...
  with tf.variable_scope('conv2') as scope:
    ...
    tf.add_to_collection('relu2', relu2)
  ...
  with tf.variable_scope('conv3') as scope:
    conv3 = tf.nn.conv3d(relu2, Wb['W3'], strides=[1, 1, 1, 1, 1], padding='VALID')
    relu3 = tf.nn.relu(tf.nn.bias_add(conv3, Wb['b3']))
    tf.add_to_collection('relu3', relu3)
  ...
  with tf.variable_scope('conv4') as scope:
    ...
    tf.add_to_collection('relu4', relu4)
  ...
  #with tf.variable_scope('conv5') as scope:
    ...
    #tf.add_to_collection('relu5', relu5)
...
def lunaTrain(train_size, test_size):
  ...
  fqbt, rbt = init_bin_file([("./data/ggo/com%d.bin" % i) for i in range(2)])
  train_label_node, train_data_node = get_train_data(fqbt, rbt)
  test_data, test_label = readTestData()
  #test_data = zoom(test_data, (1, float(32) / 40, float(32) / 40, float(32) / 40, 1))
  ...
  #compute the loss
  loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=labels_node, logits=logits), name='loss')
  ...
  with tf.Session() as sess:
    ...
    sess.run(init)
    #path = './models-lobulation-32-0.12500/model.ckpt-12489'
    #path = './models-spiculation-32-0.10590/model.ckpt-9864'
    #path = './pretrain/model.ckpt-30000'
    ...
    try:
      while not coord.should_stop():
        ...
        for step in range(int(FLAGS.NUM_EPOCHS * train_size) // FLAGS.BATCH_SIZE):
          ...
          #train_data = zoom(train_data, (1, float(32)/40, float(32)/40, float(32)/40, 1))
...
def main(_):
  train_size = 1480 #{7994:536, 17012:738, 4514:478, 14610:538， 5510：561, 9291:586, 650814:2000, 99832:2219}
  test_size = 740
