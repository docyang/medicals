...
def get_patch_with_padding(simage, center, shape=None, size=None):
    ...
    if shape:
        if np.ndim(image_array) != len(shape):
    ...
    if patch.shape != tuple(shape):
        ...
        image_pad = np.pad(image_array, pad_width=npad, mode='constant', constant_values=0)
...
def interpolatefilter(inputimage, mask):
    ...
    outputimage = resamplefilter.Execute(mask)
